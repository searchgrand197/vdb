const FORM_FIELDS = new Set(['name', 'code', 'description', 'is_active']);
...
/**
 * Parses `{ success: false, errors: { name: [...], code: [...] } }` from department create/update.
 * @returns {{ fields: Record<string, string>, general: string | null }}
 */
export function parseDepartmentFieldErrors(err) {
  ...
  /** Delete-conflict payloads use linked_*; do not map those onto the form. */
  const hasLinkedKeys =
